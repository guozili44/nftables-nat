...
install_global_command() {
    ...
    crontab -l 2>/dev/null | grep -vE "ssr auto_update|ssr auto_task|ssr daemon_check|ssr hot_upgrade|ssr clean|ssr daily_task|ssr ddns" | crontab -
    ...
    if [[ -f "$DDNS_CONF" ]] && ! crontab -l 2>/dev/null | grep -q "ssr ddns"; then 
...
force_kill_service() {
    ...
    systemctl stop "$target" 2>/dev/null; systemctl disable "$target" 2>/dev/null
    rm -f "/etc/systemd/system/${target}.service" "/etc/systemd/system/${target}"
    systemctl daemon-reload; echo -e "${GREEN}✅ 目标服务 [${target}] 已彻底蒸发！${RESET}"
    [[ "$from_menu" == "menu" ]] && sleep 2 || exit 0
}

# ==========================================================
# 核心功能模块: 原生 Cloudflare DDNS (双向联动)
# ==========================================================
setup_cf_ddns() {
    clear; echo -e "${CYAN}========= 🌐 原生 Cloudflare DDNS 配置 =========${RESET}"
    echo -e "${YELLOW}前提准备：您需要拥有一个托管在 Cloudflare 的域名，并获取您的 API Token。${RESET}\n"
    read -rp "1. 请输入 Cloudflare API Token: " cf_token; [[ -z "$cf_token" ]] && return
    read -rp "2. 请输入根域名 (例如: example.com): " cf_zone; [[ -z "$cf_zone" ]] && return
    read -rp "3. 请输入你要绑定的 DDNS 子域名 (例如: ddns.example.com): " cf_record; [[ -z "$cf_record" ]] && return

    echo -e "${CYAN}>>> 正在通过 API 验证身份并获取 Zone ID...${RESET}"
...
run_cf_ddns() {
    ...
    local success=$(echo "$api_result" | jq -r '.success')
...
unified_node_manager() {
    while true; do
        ...
        for s in /etc/systemd/system/shadowtls-*.service; do [[ -e "$s" ]] || continue; local check_port=$(basename "$s" | sed 's/shadowtls-//g' | sed 's/.service//g'); if ! [[ "$check_port" =~ ^[0-9]+$ ]]; then systemctl stop "$(basename "$s")" 2>/dev/null; systemctl disable "$(basename "$s")" 2>/dev/null; rm -f "$s"; systemctl daemon-reload; fi; done
        ...
        echo -e "${CYAN}--------------------------------------------${RESET}\n${RED} 4) ☢️ 全局强制核爆 (清理任意卡死/幽灵服务)${RESET}\n 0) 返回主菜单"; read -rp "请选择 [0-4]: " node_choice
        case "$node_choice" in
            1) if [[ $has_ss -eq 1 ]]; then clear; local ip=$(curl -s4m8 ip.sb || curl -s4m8 ifconfig.me); local port=$(jq -r '.server_port' /etc/ss-rust/config.json); local method=$(jq -r '.method' /etc/ss-rust/config.json); local password=$(jq -r '.password' /etc/ss-rust/config.json); local b64=$(echo -n "${method}:${password}" | base64 -w 0); local link="ss://${b64}@${ip}:${port}#SS-Rust"; echo -e "IP: ${GREEN}${ip}${RESET}\n端口: ${GREEN}${port}${RESET}\n协议: ${GREEN}${method}${RESET}\n密码: ${GREEN}${password}${RESET}\n${YELLOW}链接:${RESET}\n${link}\n---------------------------------\n${YELLOW}1) 修改端口 | 2) 修改密码 | ${RED}3) 删除节点${RESET} | 0) 返回"; read -rp "输入操作: " op; if [[ "$op" == "1" ]]; then read -rp "新端口: " np; jq --argjson p "$np" '.server_port = $p' /etc/ss-rust/config.json > /tmp/tmp.json && mv -f /tmp/tmp.json /etc/ss-rust/config.json; remove_firewall_rule "$port" "both"; if command -v ufw >/dev/null 2>&1; then ufw allow "$np"/tcp >/dev/null 2>&1; ufw allow "$np"/udp >/dev/null 2>&1; fi; if command -v firewall-cmd >/dev/null 2>&1; then firewall-cmd --add-port="$np"/tcp --permanent >/dev/null 2>&1; firewall-cmd --add-port="$np"/udp --permanent >/dev/null 2>&1; firewall-cmd --reload >/dev/null 2>&1; fi; systemctl restart ss-rust 2>/dev/null; echo -e "${GREEN}✅ 修改成功${RESET}"; sleep 1; elif [[ "$op" == "2" ]]; then read -rp "新密码: " npwd; jq --arg pwd "$npwd" '.password = $pwd' /etc/ss-rust/config.json > /tmp/tmp.json && mv -f /tmp/tmp.json /etc/ss-rust/config.json; systemctl restart ss-rust 2>/dev/null; echo -e "${GREEN}✅ 修改成功${RESET}"; sleep 1; elif [[ "$op" == "3" ]]; then remove_firewall_rule "$port" "both"; systemctl stop ss-rust 2>/dev/null; systemctl disable ss-rust 2>/dev/null; rm -rf /etc/ss-rust /usr/local/bin/ss-rust /etc/systemd/system/ss-rust.service; systemctl daemon-reload; echo -e "${GREEN}✅ 已彻底销毁！${RESET}"; sleep 1; fi; fi ;;
            2) if [[ $has_vless -eq 1 ]]; then clear; local ip=$(curl -s4m8 ip.sb || curl -s4m8 ifconfig.me); local port=$(jq -r '.inbounds[0].port' /usr/local/etc/xray/config.json); local uuid=$(jq -r '.inbounds[0].settings.clients[0].id' /usr/local/etc/xray/config.json); local sni=$(jq -r '.inbounds[0].streamSettings.realitySettings.serverNames[0]' /usr/local/etc/xray/config.json); echo -e "IP: ${GREEN}${ip}${RESET}\n端口: ${GREEN}${port}${RESET}\nUUID: ${GREEN}${uuid}${RESET}\nSNI伪装: ${GREEN}${sni}${RESET}\n---------------------------------\n${YELLOW}1) 重启节点 | ${RED}2) 删除节点${RESET} | 0) 返回"; read -rp "输入操作: " op; if [[ "$op" == "1" ]]; then systemctl restart xray 2>/dev/null; echo -e "${GREEN}✅ 已重启${RESET}"; sleep 1; elif [[ "$op" == "2" ]]; then remove_firewall_rule "$port" "tcp"; systemctl stop xray 2>/dev/null; systemctl disable xray 2>/dev/null; rm -rf /usr/local/etc/xray /usr/local/bin/xray /etc/systemd/system/xray.service; systemctl daemon-reload; echo -e "${GREEN}✅ 已彻底销毁！${RESET}"; sleep 1; fi; fi ;;
            3) if [[ $has_stls -eq 1 ]]; then clear; local st_ports=(); local idx=1; for s in /etc/systemd/system/shadowtls-*.service; do [[ -e "$s" ]] || continue; local st_port=$(basename "$s" | sed 's/shadowtls-//g' | sed 's/.service//g'); st_ports[$idx]=$st_port; local st_status=$(systemctl is-active --quiet shadowtls-"$st_port" && echo -e "${GREEN}运行中${RESET}" || echo -e "${RED}已停止${RESET}"); echo -e " ${CYAN}${idx})${RESET} 端口: ${YELLOW}${st_port}${RESET} [${st_status}]"; ((idx++)); done; echo -e "---------------------------------\n${RED}1) 序号删除实例 | 9) ⚠️ 强制核爆所有残留${RESET} | 0) 返回"; read -rp "输入操作: " op; if [[ "$op" == "1" ]]; then read -rp "输入实例序号 [1-$((idx-1))]: " del_idx; local del_port=${st_ports[$del_idx]}; if [[ -n "$del_port" && -f "/etc/systemd/system/shadowtls-${del_port}.service" ]]; then remove_firewall_rule "$del_port" "tcp"; systemctl stop shadowtls-"$del_port" 2>/dev/null; systemctl disable shadowtls-"$del_port" 2>/dev/null; rm -f "/etc/systemd/system/shadowtls-${del_port}.service"; systemctl daemon-reload; if ! ls /etc/systemd/system/shadowtls-*.service 1> /dev/null 2>&1; then rm -f /usr/local/bin/shadow-tls; fi; echo -e "${GREEN}✅ 已彻底销毁！${RESET}"; sleep 1; fi; fi; if [[ "$op" == "9" ]]; then echo -e "${RED}执行物理核爆...${RESET}"; for p in "${st_ports[@]}"; do remove_firewall_rule "$p" "tcp"; systemctl stop "shadowtls-$p" 2>/dev/null; systemctl disable "shadowtls-$p" 2>/dev/null; rm -f "/etc/systemd/system/shadowtls-${p}.service"; done; rm -f /usr/local/bin/shadow-tls; systemctl daemon-reload; echo -e "${GREEN}✅ 拔除成功！${RESET}"; sleep 2; fi; fi ;;
            4) clear; echo -e "${CYAN}========= ☢️ 全局强制核爆中心 =========${RESET}\n${YELLOW}支持强制抹除系统中任何卡死、报错或残留的服务。${RESET}\n参考名：${GREEN}ss-rust${RESET} | ${GREEN}xray${RESET} | ${GREEN}shadowtls-错误端口${RESET}\n---------------------------------"; read -rp "请输入要粉碎的服务名 (直接回车取消): " nuke_target; [[ -n "$nuke_target" ]] && force_kill_service "$nuke_target" "menu" ;;
            0) return ;;
        esac
    done
}
...
hot_update_components() {
    ...
    if [[ -x "/usr/local/bin/ss-rust" ]]; then local ss_latest=$(curl -s --max-time 10 https://api.github.com/repos/shadowsocks/shadowsocks-rust/releases/latest | grep '"tag_name":' | grep -oE 'v[0-9]+\.[0-9]+\.[0-9]+' | head -n 1); [[ -n "$ss_latest" ]] && wget -qO /tmp/ss-rust.tar.xz "https://github.com/shadowsocks/shadowsocks-rust/releases/download/${ss_latest}/shadowsocks-${ss_latest}.${ss_arch}.tar.xz" && tar -xf /tmp/ss-rust.tar.xz -C /tmp/ ssserver 2>/dev/null && mv -f /tmp/ssserver /usr/local/bin/ss-rust && systemctl restart ss-rust 2>/dev/null; fi
    if [[ -x "/usr/local/bin/shadow-tls" ]]; then local st_latest=$(curl -s --max-time 10 https://api.github.com/repos/ihciah/shadow-tls/releases/latest | grep '"tag_name":' | grep -oE 'v[0-9]+\.[0-9]+\.[0-9]+' | head -n 1); [[ -n "$st_latest" ]] && wget -qO /tmp/shadow-tls "https://github.com/ihciah/shadow-tls/releases/download/${st_latest}/shadow-tls-${st_arch}" && mv -f /tmp/shadow-tls /usr/local/bin/shadow-tls && chmod +x /usr/local/bin/shadow-tls && for s in $(systemctl list-units --type=service --all --no-legend | grep "shadowtls-" | awk '{print $1}'); do systemctl restart "$s" 2>/dev/null; done; fi
    if [[ -x "/usr/local/bin/xray" ]]; then local xr_latest=$(curl -s --max-time 10 https://api.github.com/repos/XTLS/Xray-core/releases/latest | grep '"tag_name":' | grep -oE 'v[0-9]+\.[0-9]+\.[0-9]+' | head -n 1); [[ -n "$xr_latest" ]] && wget -qO /tmp/xray.zip "https://github.com/XTLS/Xray-core/releases/download/${xr_latest}/Xray-linux-${xray_arch}.zip" && unzip -qo /tmp/xray.zip xray -d /tmp/ 2>/dev/null && mv -f /tmp/xray /usr/local/bin/xray && systemctl restart xray 2>/dev/null; fi
...
total_uninstall() {
    ...
    if [[ -f "$DDNS_CONF" ]]; then remove_cf_ddns "force"; fi
    rm -f /usr/local/bin/shadow-tls "$CONF_FILE" "$NAT_CONF_FILE" "$DDNS_CONF" "$DDNS_LOG" /usr/local/bin/ssr /usr/local/bin/ssr.sh; crontab -l 2>/dev/null | grep -vE "ssr auto_update|ssr auto_task|ssr daemon_check|ssr hot_upgrade|ssr clean|ssr daily_task|ssr ddns" | crontab -
