...
NFTABLES_CREATED_MARK="/etc/nftables.conf.nft_created"
...
remove_ddns_cron_task() {
    # 删除所有 nft --cron 的 crontab 行（避免路径差异导致残留）
    local cur
    cur="$(crontab -l 2>/dev/null || true)"
    [[ -z "$cur" ]] && return 0
    echo "$cur" | grep -vE '(^|\s)(/usr/local/bin/nft|nft)\s+--cron(\s|$)' | crontab - 2>/dev/null || true
    return 0
}
...
ensure_ddns_cron_disabled_if_unused() {
    ...
    if crontab -l 2>/dev/null | grep -Fq "${script_path} --cron" || crontab -l 2>/dev/null | grep -Eq '(^|\s)(/usr/local/bin/nft|nft)\s+--cron(\s|$)'; then
        remove_ddns_cron_task
...
EOF
        ...
        # 备份并注入 include
        local bak="${NFTABLES_CONF}.nft.bak.$(date +%s)"
...
persist_status() {
    ...
    if [[ -f "$NFTABLES_CONF" ]]; then
        if nftables_conf_includes_mgr; then
            echo -e "/etc/nftables.conf: ${GREEN}已包含 nft 规则（include）${PLAIN}"
        else
            echo -e "/etc/nftables.conf: ${YELLOW}未包含 nft include${PLAIN}"
        fi
    else
...
[Unit]
Description=nftables Port Forwarding Manager (nft)
...
add_forward_impl() {
    ...
    local conf_bak
    conf_bak="$(mktemp /tmp/nft-conf.XXXXXX)"
...
ddns_update_impl() {
    local changed=0
    local temp_file
    temp_file="$(mktemp /tmp/nft-ddns.XXXXXX)"
...
uninstall_script_impl() {
    ...
    if [[ -f "$NFTABLES_CONF" ]]; then
        sed -i '/# nft include (added .*$/d' "$NFTABLES_CONF" 2>/dev/null || true
        sed -i '/# nft persistent include$/d' "$NFTABLES_CONF" 2>/dev/null || true
    ...
    if [[ -f "$NFTABLES_CREATED_MARK" ]]; then
        ...
        local latest_bak
        latest_bak="$(ls -1t ${NFTABLES_CONF}.nft.bak.* 2>/dev/null | head -n 1)"
    ...
    # 7) 删除我们创建的备份（仅 nft 命名的）
    rm -f ${NFTABLES_CONF}.nft.bak.* 2>/dev/null || true
